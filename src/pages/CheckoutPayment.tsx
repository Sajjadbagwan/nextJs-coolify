...
import { Lock, Pencil, ShieldCheck, CreditCard } from "lucide-react";
...
const CheckoutPayment = () => {
  const [method, setMethod] = useState<"card" | "applepay" | "googlepay">("card");
  const [sameAsShipping, setSameAsShipping] = useState(true);
  const [saveInfo, setSaveInfo] = useState(false);
  const [openSheet, setOpenSheet] = useState<null | "terms" | "privacy">(null);

  // Demo total mirroring summary calc (no shipping)
  const total = 352.73;
  ...
  return (
    <div className="min-h-screen bg-background w-full max-w-[640px] mx-auto overflow-x-hidden pb-40">
      ...
      <section className="px-4 pt-5 space-y-2">
        {[
          { label: "Contact", value: "john.smith@example.com", to: "/checkout/shipping" },
          { label: "Ship to", value: "4 Ashcroft Road, Rochester, Kent ME3 8GS", to: "/checkout/shipping" },
      ...
      <section className="px-4 mt-6">
        ...
        <div className="space-y-2">
          {/* Apple Pay */}
          <label
            className={`block rounded-lg border-2 transition-colors ${
              method === "applepay" ? "border-[hsl(var(--brand-mid))] bg-[hsl(var(--brand-bg))]" : "border-border"
            }`}
          >
          ...
          <label
            className={`block rounded-lg border-2 transition-colors ${
              method === "card" ? "border-[hsl(var(--brand-mid))] bg-[hsl(var(--brand-bg))]" : "border-border"
            }`}
          >
            <div className="flex items-center gap-3 p-3 cursor-pointer">
      ...
      <section className="px-4 mt-6">
        ...
        <div className="space-y-2">
          <label className={`flex items-center gap-3 p-3 rounded-lg border-2 ${sameAsShipping ? "border-[hsl(var(--brand-mid))] bg-[hsl(var(--brand-bg))]" : "border-border"}`}>
            <input type="radio" checked={sameAsShipping} onChange={() => setSameAsShipping(true)} className="w-4 h-4 accent-[hsl(var(--brand-mid))]" />
            <span className="text-[14px] font-medium">Same as shipping address</span>
          </label>
          <label className={`flex items-center gap-3 p-3 rounded-lg border-2 ${!sameAsShipping ? "border-[hsl(var(--brand-mid))] bg-[hsl(var(--brand-bg))]" : "border-border"}`}>
            <input type="radio" checked={!sameAsShipping} onChange={() => setSameAsShipping(false)} className="w-4 h-4 accent-[hsl(var(--brand-mid))]" />
            <span className="text-[14px] font-medium">Use a different billing address</span>
          </label>
        </div>
        {!sameAsShipping && <BillingAddressForm />}
      </section>

      {/* Remember me */}
      <section className="px-4 mt-6">
        <h2 className="font-heading text-base font-medium text-[hsl(var(--brand-dark))] uppercase tracking-wide mb-3">Remember Me</h2>
        <label className="flex items-center gap-2 text-sm">
          <input type="checkbox" checked={saveInfo} onChange={(e) => setSaveInfo(e.target.checked)} className="w-4 h-4 accent-[hsl(var(--brand-mid))]" />
          Save my information for a faster checkout
        </label>
        {saveInfo && (
          <div className="grid grid-cols-2 gap-2 mt-2">
            <input type="password" placeholder="Password" className="h-11 px-3 text-sm bg-background border border-border rounded focus:outline-none focus:ring-2 focus:ring-[hsl(var(--brand-light-blue))]" />
            <input type="password" placeholder="Confirm password" className="h-11 px-3 text-sm bg-background border border-border rounded focus:outline-none focus:ring-2 focus:ring-[hsl(var(--brand-light-blue))]" />
          </div>
        )}
      </section>
